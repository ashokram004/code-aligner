...
def get_ai_feedback(user_code, lang, issue_type, context):
    ...
    try:
        ...
        return "⚠️ AI Traffic Limit Reached. Please wait 60s."
    except Exception as e:
        return f"AI Error: {e}"

def run_universal_pipeline(user_code, problem_desc=""):
    print("\n" + "="*60)
    print("🚀 STARTING AI CODE ALIGNER")
    print("="*60)

    # --- PHASE 1: INSPECTION (The Detective) ---
    # Pass description to Inspector
    user_meta = inspect_code_snippet(user_code, problem_desc, API_KEY)
    if not user_meta: 
    ...
    predicted_slug = user_meta.get('predicted_slug', 'Unknown')
    test_inputs_str = user_meta.get('test_input')
    
    print(f"🔹 Language: {lang.upper()}")
    print(f"🔹 User Function: '{user_func}'")
    print(f"🔹 Target Problem: '{predicted_slug}'")
    ...
    # HYBRID SEARCH: Pass the AI's predicted slug to boost accuracy
    golden_code, conf = find_solution(user_code, predicted_slug=predicted_slug)
    
    # If conf > 0.9, it means we found an EXACT slug match
    if conf > 0.9:
        print(f"Match Found! (Type: Exact Slug Match)")
        
        if lang == 'python':
            # Inspect golden code to find function name
            print("   [inspector] Scanning Golden Solution...")
            gold_meta = inspect_code_snippet(golden_code, "", API_KEY)
            
            if gold_meta:
                gold_func = gold_meta.get('user_function') or "twoSum"
        ...
        print(f"⚠️ No database match found for '{predicted_slug}'.")
...
if __name__ == "__main__":
    ...
    # TEST 1: Obscure function name + Description
    obscure_code = """
def magic_pair_finder(arr, target):
    ...
    return []
"""
    desc = "Find two numbers in the array that add up to the target."
    
    run_universal_pipeline(obscure_code, desc)